...
@section('title', 'Razen Teknologi | Layanan Pemerintahan | Data | Produk Unggulan')
...
@section('content')
    ...
    <div class="page-title-container">
        ...
        <div class="col-12 col-md-7">
            <h1 class="mb-0 pb-0 display-4" id="title"> Produk Unggulan</h1>
            <nav class="breadcrumb-container d-inline-block" aria-label="breadcrumb">
                <ul class="breadcrumb pt-0">
                    ...
                    <li class="breadcrumb-item"><a href="{{ route('razen-teknologi.landing-page.layanan.pemerintahan.data.produk-unggulan.index') }}">Produk Unggulan</a></li>
                </ul>
            </nav>
        </div>
        <!-- Title End -->
    ...
    <div class="data-table-rows slim">
        ...
        <div class="data-table-responsive-wrapper">
            <table id="produk_unggulan_table" class="data-table w-100">
                <thead>
                    <tr>
                        <th>No</th>
                        <th>Judul</th>
        ...
        <div class="modal-dialog modal-dialog-centered modal-lg">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <form class="form-horizontal" id="produk_unggulan_form" method="POST" enctype="multipart/form-data">
                        @csrf
...
@section('js')
    ...
    <script>
        $(document).ready(function(){
            ...
            var dataTables = $('#produk_unggulan_table').DataTable({
                processing: true,
                serverSide: true,
                ajax: {
                    url: "{{ route('razen-teknologi.landing-page.layanan.pemerintahan.data.produk-unggulan.index') }}"
                },
                columns: [
                    {
                        data: 'DT_RowIndex',
        ...
        $('#produk_unggulan_form').on('submit', function(e){
            e.preventDefault();
            if($('#aksi').val() == 'Save')
            {
                $.ajax({
                    url: "{{ route('razen-teknologi.landing-page.layanan.pemerintahan.data.produk-unggulan.store') }}",
                    method: "POST",
                    ...
                    success: function(data)
                    {
                        ...
                        if(data.success)
                        {
                            ...
                            $('#produk_unggulan_form')[0].reset();
                            $('#aksi_button').text('Save');
                            $('#produk_unggulan_table').DataTable().ajax.reload();
                        }
            ...
            {
                $.ajax({
                    url: "{{ route('razen-teknologi.landing-page.layanan.pemerintahan.data.produk-unggulan.update') }}",
                    method: "POST",
                    data: new FormData(this),
                    dataType: "json",
                    contentType: false,
        ...
        $(document.body).on('click', '.detail', function(){
            var id = $(this).attr('id');
            var url = "{{ route('razen-teknologi.landing-page.layanan.pemerintahan.data.produk-unggulan.show', ['id' => ":id"]) }}";
            ...
            $.ajax({
                ...
                success: function(data)
                {
                    ...
                    $('#detail_gambar').attr('src', "{{ asset('images/landing-page/layanan/pemerintahan/data/produk-unggulan') }}" + '/' + data.result.gambar);
                    $('#detail').modal('show');
                }
            });
        });
        ...
        $(document).on('click', '.edit', function(){
            var id = $(this).attr('id');
            $('#form_result').html('');
            var url = "{{ route('razen-teknologi.landing-page.layanan.pemerintahan.data.produk-unggulan.edit', ['id' => ":id"]) }}";
            url = url.replace(':id', id);
            $.ajax({
                url: url,
                dataType: "json",
                success: function(data)
                {
                    ...
                    var lokasi_gambar = "{{ asset('images/landing-page/layanan/pemerintahan/data/produk-unggulan') }}"+'/'+data.result.gambar;
        ...
        $('#ok_button').click(function(){
            var url = "{{ route('razen-teknologi.landing-page.layanan.pemerintahan.data.produk-unggulan.destroy', ['id' => ":id"]) }}";
            url = url.replace(":id", id);
            $.ajax({
                url: url,
                beforeSend: function(){
