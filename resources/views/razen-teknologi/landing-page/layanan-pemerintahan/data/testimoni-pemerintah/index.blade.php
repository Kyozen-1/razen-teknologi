...
@section('title', 'Razen Teknologi | Layanan Pemerintahan | Data | Testimoni Pemerintah')
...
@section('content')
    ...
    <div class="page-title-container">
        ...
        <div class="col-12 col-md-7">
            <h1 class="mb-0 pb-0 display-4" id="title"> Testimoni Pemerintah</h1>
            <nav class="breadcrumb-container d-inline-block" aria-label="breadcrumb">
                <ul class="breadcrumb pt-0">
                    <li class="breadcrumb-item"><a href="{{ route('razen-teknologi.admin.dashboard.index') }}">Dashboard</a></li>
                    <li class="breadcrumb-item"><a href="#">Layanan Pemerintahan</a></li>
                    <li class="breadcrumb-item"><a href="#">Data</a></li>
                    <li class="breadcrumb-item"><a href="{{ route('razen-teknologi.landing-page.layanan.pemerintahan.data.testimoni-pemerintah.index') }}">Testimoni Pemerintah</a></li>
                </ul>
            </nav>
        </div>
        <!-- Title End -->
    ...
    <div class="data-table-rows slim">
        ...
        <div class="data-table-responsive-wrapper">
            <table id="testimoni_pemerintah_table" class="data-table w-100">
                <thead>
                    <tr>
                        <th>No</th>
                        <th>Nama</th>
        ...
        <div class="modal-dialog modal-dialog-centered modal-lg">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <form class="form-horizontal" id="testimoni_pemerintah_form" method="POST" enctype="multipart/form-data">
                        @csrf
                        ...
                        <div class="mb-3 position-relative form-group">
                            <label for="judul_testimoni" class="control-label">Judul Testimoni</label>
                            <input type="text" class="form-control" name="judul_testimoni" id="judul_testimoni" required>
                        </div>
                        <div class="mb-3 position-relative form-group">
        ...
        <div class="modal-dialog modal-dialog-centered modal-lg">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <div class="form-group mb-3 position-relative">
                        <label for="detail_judul_testimoni" class="control-label">Judul Testimoni</label>
                        <input type="text" id="detail_judul_testimoni" class="form-control" disabled>
                    </div>
                    <div class="mb-3 position-relative form-group">
                        <label for="testimoni" class="control-label">Testimoni</label>
                        <textarea id="detail_testimoni" rows="5" class="form-control" disabled></textarea>
                    </div>
                </div>
            </div><!-- /.modal-content -->
        </div><!-- /.modal-dialog -->
...
@section('js')
    ...
    <script>
        $(document).ready(function(){
            ...
            var dataTables = $('#testimoni_pemerintah_table').DataTable({
                processing: true,
                serverSide: true,
                ajax: {
                    url: "{{ route('razen-teknologi.landing-page.layanan.pemerintahan.data.testimoni-pemerintah.index') }}"
                },
                columns: [
                    {
                        data: 'DT_RowIndex',
        ...
        $('#create').click(function(){
            $('#testimoni_pemerintah_form')[0].reset();
            $('.dropify-clear').click();
        ...
        $('#testimoni_pemerintah_form').on('submit', function(e){
            e.preventDefault();
            if($('#aksi').val() == 'Save')
            {
                $.ajax({
                    url: "{{ route('razen-teknologi.landing-page.layanan.pemerintahan.data.testimoni-pemerintah.store') }}",
                    method: "POST",
                    ...
                    success: function(data)
                    {
                        ...
                        if(data.success)
                        {
                            ...
                            $('#testimoni_pemerintah_form')[0].reset();
                            $('#aksi_button').text('Save');
                            $('#testimoni_pemerintah_table').DataTable().ajax.reload();
                        }
            ...
            {
                $.ajax({
                    url: "{{ route('razen-teknologi.landing-page.layanan.pemerintahan.data.testimoni-pemerintah.update') }}",
                    method: "POST",
                    data: new FormData(this),
                    dataType: "json",
                    contentType: false,
        ...
        $(document.body).on('click', '.detail', function(){
            var id = $(this).attr('id');
            var url = "{{ route('razen-teknologi.landing-page.layanan.pemerintahan.data.testimoni-pemerintah.show', ['id' => ":id"]) }}";
            ...
            $.ajax({
                ...
                success: function(data)
                {
                    ...
                    $('#detail_foto').attr('src', "{{ asset('images/landing-page/layanan/pemerintahan/data/testimoni-pemerintah') }}" + '/' + data.result.foto);
                    $('#detail_judul_testimoni').val(data.result.judul_testimoni);
                    $('#detail_testimoni').val(data.result.testimoni);
                    $('#detail').modal('show');
                }
            });
        });
        ...
        $(document).on('click', '.edit', function(){
            var id = $(this).attr('id');
            $('#form_result').html('');
            var url = "{{ route('razen-teknologi.landing-page.layanan.pemerintahan.data.testimoni-pemerintah.edit', ['id' => ":id"]) }}";
            url = url.replace(':id', id);
            $.ajax({
                url: url,
                dataType: "json",
                success: function(data)
                {
                    ...
                    $('#judul_testimoni').val(data.result.judul_testimoni);
                    $('#testimoni').val(data.result.testimoni);
                    var lokasi_foto = "{{ asset('images/landing-page/layanan/pemerintahan/data/testimoni-pemerintah') }}"+'/'+data.result.foto;
        ...
        $('#ok_button').click(function(){
            var url = "{{ route('razen-teknologi.landing-page.layanan.pemerintahan.data.testimoni-pemerintah.destroy', ['id' => ":id"]) }}";
            url = url.replace(":id", id);
            $.ajax({
                url: url,
                beforeSend: function(){
